  ...
  <div>
    <div class="hello">
      <div
        ref="hello"
        class="hello__inner"
        :class="{ _status_done: typingMotion.status === 'done' }"
      ></div>
    </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('typingMotion', {
      getTypingMotion: 'getItemByName'
    }),

    typingMotion() {
      return this.getTypingMotion('intro')
    }
  },

  watch: {
    'typingMotion.shot'(val) {
      this.$refs.hello.innerHTML = `${val}<span class="hello__cursor">|</span>`
    },
    authUser(val) {
      if (val) {
        this.runTypingMotion('intro')
      }
    }
  },

  created() {
    this.addTypingMotion({ name: 'intro', resource: typingMotionData })
  },
